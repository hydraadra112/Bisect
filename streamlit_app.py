...
def get_ba_and_within(df: pd.DataFrame, tolerance: Real) -> pd.DataFrame:
    ...
    for i in range(len(df)):
        ...
        additional_data.append({"|b-a|": val, "within_tolerance": within})
...
def plot_function(f: Callable, a: Real, b: Real, root: Real) -> None:
    ...
    source = pd.DataFrame({"x": x_range, "f(x)": y_range})

    # B. Create Data for the Root Marker
    root_data = pd.DataFrame({"x": [root], "f(x)": [f(root)]})
    ...
    line_chart = (
        alt.Chart(source)
        .mark_line()
        .encode(x=alt.X("x", title="x"), y=alt.Y("f(x)", title="f(x)"))
    )
    ...
    a_marker = (
        alt.Chart(pd.DataFrame({"a": [a]}))
        .mark_rule(color="green", strokeDash=[5, 5], size=1.5)
        .encode(
            x="a", tooltip=[alt.Tooltip("a", title="a (Interval Start)", format=".5f")]
        )
    )
    ...
    b_marker = (
        alt.Chart(pd.DataFrame({"b": [b]}))
        .mark_rule(color="blue", strokeDash=[5, 5], size=1.5)
        .encode(
            x="b", tooltip=[alt.Tooltip("b", title="b (Interval End)", format=".5f")]
        )
    )

    # D. Combine and Display - Add the new markers
    st.altair_chart(
        (line_chart + zero_line + point_chart + a_marker + b_marker).interactive(),
        width="stretch",
    )
...
def main():
    ...
    st.caption(
        "We recommend using Python syntax when defining your equations. Trig functions are supported: `sin(x)`, `cos(x)`, `exp(x)`."
    )
    ...
    with col2:
        tolerance = st.number_input(
            "Tolerance ℰ", value=0.0001, format="%.5f", step=0.0001
        )
